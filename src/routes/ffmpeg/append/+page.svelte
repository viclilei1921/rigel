<script lang="ts">
  ...
  async function handleAppend() {
    ...
    const newInputs = videoArr.map((item) => item.path);
    newInputs.shift();

    await invoke('append_smart', {
      basePath: videoArr[0].path,
      newInputs,
...
<div class="flex flex-col gap-2 p-2">
  <div>
    使用 ffmpeg 将多个视频合成为一个视频, 以第一个视频为基准将后续视频处理(包括文件名绘制)后追加到第一个视频后
  </div>
  <div class="flex gap-2">
    ...
    {#if videoArr.length > 0}
      ...
      >
        {#if !appending}
          <ArrowSwitch />
          <span class="text-align-left w-18">开始合成</span>
        {:else}
          <ButtonLoading />
          <span class="text-align-left w-18">合成中...</span>
        {/if}
      </button>
    {/if}
  </div>
  {#if videoArr.length > 0}
    <div
      class="flex flex-col items-center overflow-hidden rounded-sm border border-gray-400 pt-2 shadow-sm transition-shadow hover:shadow-md"
    >
